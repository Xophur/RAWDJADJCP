  ...
  if (!ch) {
    return (
      <div className="pt-32 pb-40 text-center">
        ...
        <Link to="/" className="text-neon-green underline">Back home</Link>
      </div>
    );
  }
  ...
  return (
    <article data-testid={`chapter-${ch.id}`} className="relative z-10">
      ...
      <div className="relative h-[55vh] min-h-[360px]">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-4xl mx-auto px-5 sm:px-8 w-full pb-12">
            <Link to="/" className="inline-flex items-center gap-1.5 text-white/60 hover:text-neon-green text-xs uppercase tracking-wide font-semibold mb-5">
      ...
      <div className="max-w-2xl mx-auto px-5 sm:px-8 py-16">
        {ch.sections.map((s, i) => (
          <Reveal key={i} className="mb-12">
            <h2 className="font-display font-bold uppercase tracking-tight text-2xl sm:text-3xl text-neon-green mb-5">
              {s.heading}
            </h2>
            ...
            {s.pullQuote && (
              <blockquote className="my-8 pl-5 border-l-4 border-neon-orange">
                <p className="font-display uppercase tracking-tight text-xl sm:text-2xl leading-tight text-white">
                  “{s.pullQuote}”
                </p>
              </blockquote>
            )}
        ...
        {/* Goblin Mode gallery on the bonus chapter */}
        {ch.id === "corporate-vs-real" && (
          <Reveal className="my-12">
            ...
            <div className="grid grid-cols-2 gap-4">
              {RAVE_PHOTOS.map((p, i) => (
                <figure key={i} className={`rounded-md overflow-hidden border border-white/10 ${i === 0 ? "col-span-2" : ""}`}>
        ...
        {ch.terms?.length > 0 && (
          <div className="mt-12 pt-8 border-t border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              {ch.terms.map((t) => (
                <span key={t} className="font-mono-x text-xs uppercase tracking-wide text-white/70 border border-white/15 px-3 py-1.5 rounded-sm">
                  {t}
                </span>
              ))}
            </div>
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-8 pb-20 grid sm:grid-cols-2 gap-4">
        {prev ? (
          <Link to={`/chapter/${prev.id}`} data-testid="chapter-prev" className="group bg-cardp border border-white/10 rounded-md p-6 hover:border-neon-green transition-colors">
        ...
        {next ? (
          <Link to={`/chapter/${next.id}`} data-testid="chapter-next" className="group bg-cardp border border-white/10 rounded-md p-6 hover:border-neon-orange transition-colors sm:text-right">
          ...
          <Link to="/certificate" data-testid="chapter-finish" className="group bg-neon-orange text-black rounded-md p-6 hover:brightness-110 transition-all sm:text-right">
